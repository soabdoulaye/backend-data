...
export const setupTwoFactor = async (user_id: string) => {
    ...
    await supabase.from('users').update({ two_factor_secret: secret.base32 }).eq('id', user_id);
...
export const verifyTwoFactor = async (user_id: string, token: string) => {
    ...
    if (!user.two_factor_enabled) {
        await supabase.from('users').update({ two_factor_enabled: true }).eq('id', user_id);
    }

    const payload = { user_id: user.id, username: user.username, role: user.role };
    const jwtToken = jwt.sign(payload, JWT_SECRET, { expiresIn: '1h' });

    await supabase.from('users').update({ last_login_at: new Date() }).eq('id', user_id);
...
export const verifyToken = (token: string): { user_id: string; username: string; role: string } => {
    try {
        return jwt.verify(token, JWT_SECRET) as { user_id: string; username: string; role: string };
    } catch {
